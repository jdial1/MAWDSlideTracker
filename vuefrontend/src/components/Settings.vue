...
export default {
  ...
  methods: {
    getValues() {
      ...
      axios
        .post(store.getters.getApiUrl + "/GetCassEngLoc", {
          action: "GetCassEngLoc",
        })
        .then((apidata) => {
          apidata.data.forEach((item) => {
            item.right_left_value = item.right_left_value ? true : false;
          });
          this.items = apidata.data;
          this.origItems = apidata.data;
    ...
    SaveAllChanges() {
      this.loading = true;
      axios
        .post(store.getters.getApiUrl + "/SetCassEngLoc", {
