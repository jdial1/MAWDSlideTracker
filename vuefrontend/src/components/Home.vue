  ...
  <div class="outer-container">
    <div class="container">
      ...
      <b-progress v-if="!this.loading" :max="max" height="8rem">
        <b-progress-bar :value="this.barValue(preEmbedded)"
          ><span
            >PreEmbeded
            <h3>{{ preEmbedded }}</h3></span
          >
    
        </b-progress-bar>
        <b-progress-bar :value="this.barValue(embedded)"
          ><span
            >Embeded
            <h3>{{ embedded }}</h3></span
          ></b-progress-bar
        >
        <b-progress-bar :value="this.barValue(Stained)"
          ><span
            >Stained
            <h3>{{ Stained }}</h3>
          </span></b-progress-bar
        >
        <b-progress-bar :value="this.barValue(slidesCut)"
          ><span
            >Slides Cut
            <h3>{{ slidesCut }}</h3>
          </span></b-progress-bar
        >
        <b-progress-bar :value="this.barValue(distributed)"
          ><span
            >Distributed
            <h3>{{ distributed }}</h3></span
          ></b-progress-bar
        >
      </b-progress>
      <b-icon
...
export default {
  ...
  methods: {
    barValue(value) {
      return value * 0.5 + this.max / 7 > 150
        ? value * 0.5 + this.max / 7
        : 150;
    },
    LoadData() {
      this.loading = true;
      axios
        .post(store.getters.getApiUrl + "/GetStatusData", {
